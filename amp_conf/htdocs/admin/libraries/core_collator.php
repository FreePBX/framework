<?php
// This file was/is part of Moodle - http://moodle.org/
//
...
class core_collator {
		...
		protected static function ensure_collator_available() {
				$locale = !empty($_COOKIE['lang'])?$_COOKIE['lang']:'en_US';
				if (is_null(self::$collator) || $locale != self::$locale) {
						...
						if (class_exists('Collator', false)) {
								...
								if (!empty($collator) && $collator instanceof Collator) {
										...
										if ($errorcode !== 0) {
												...
												if ($errorcode === -127 || $errorcode === -128) {
														...
														if ($localeinuse !== 'root' && strpos($locale, $localeinuse) !== 0) {
																...
																freepbx_log(FPBX_LOG_WARNING,'Locale warning (not fatal) '.$errormessage.': '.
														...
														freepbx_log(FPBX_LOG_WARNING,'Problem with locale: '.$errormessage.'. '.
										...
										freepbx_log(FPBX_LOG_WARNING,'Error instantiating collator for locale: "' . $locale . '", with error [' .
