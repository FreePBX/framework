...
class GPG {
	...
	/**
	 * Read expiry details from a GPG key file.
	 *
	 * @param string $path
	 * @return array {
	 *   @type bool   $valid
	 ...
	 * }
	 */
	private function getRepoKeyExpiryFromFile($path) {
		$result = [
			'valid' => false,
			'expires_in_days' => 0,
			'expiry_date' => '',
		...
		$output = [];
		$returnVar = 0;
		exec($this->gpg . ' --show-keys ' . escapeshellarg($path) . ' 2>/dev/null', $output, $returnVar);
	...
	/**
	 * Check if the FreePBX APT GPG key exists and get its expiry status.
	 *
	 * @return array {
	 *   @type bool   $applicable     True if this system uses the APT GPG key
	 *   @type bool   $needs_update   True if key expires in less than warning days
	 ...
	 * }
	 */
	public function checkRepoKeyExpiry() {
		$result = [
			'applicable' => false,
			'needs_update' => false,
			'expires_in_days' => 0,
		...
		$aptDir = dirname(self::GPG_KEY_PATH);
		if (!is_dir($aptDir)) {
			return $result;
		}
		...
		$direct = $this->getRepoKeyExpiryFromFile(self::GPG_KEY_PATH);
		if ($direct['valid']) {
			$result['expiry_date'] = $direct['expiry_date'];
			$result['expires_in_days'] = $direct['expires_in_days'];
			$result['key_id'] = $direct['key_id'];
			$result['needs_update'] = ($result['expires_in_days'] < self::GPG_KEY_EXPIRY_UPDATE_DAYS);
			return $result;
		}
	...
	/**
	 * Update the FreePBX APT GPG key by downloading from the official URL.
	 *
	 * @return array {
	...
	public function updateRepoKey() {
		...
		$response = \FreePBX::Curl()->get(self::GPG_KEY_URL);
		...
		try {
			if (file_put_contents($tmpKey, $response->body) === false || !filesize($tmpKey)) {
				...
				$output = [];
				$exitCode = 0;
				exec($this->gpg . ' --dearmor --yes -o ' . escapeshellarg($tmpGpg) . ' ' . escapeshellarg($tmpKey) . ' 2>/dev/null', $output, $exitCode);
				if ($exitCode !== 0 || !filesize($tmpGpg)) {
					$message = sprintf(_('Failed to dearmor GPG key. Exit code: %s'), $exitCode);
				} elseif (!@rename($tmpGpg, self::GPG_KEY_PATH)) {
					if (@copy($tmpGpg, self::GPG_KEY_PATH)) {
						@unlink($tmpGpg);
					} else {
	 ...
	 * @return array
	 */
	public function checkAndUpdateRepoKey($autoUpdate = false, $outputCallback = null) {
		$result = [
			'checked' => true,
			'applicable' => false,
			'needed_update' => false,
		...
		$gpgStatus = $this->checkRepoKeyExpiry();
		...
		if ($autoUpdate) {
			$result['updated'] = true;
			$updateResult = $this->updateRepoKey();
