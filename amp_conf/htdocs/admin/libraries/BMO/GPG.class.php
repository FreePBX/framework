...
class GPG {
	...
	private function ensureRepoKeyConstants() {
	...
	private function repoKeyExpiryFromFile($path) {
		$result = [
			'valid' => false,
			'expires_in_days' => 0,
			'expiry_date' => '',
	...
	private function checkRepoKeyExpiry() {
		$result = [
			'applicable' => false,
			'needs_update' => false,
			'expires_in_days' => 0,
		...
		$direct = $this->repoKeyExpiryFromFile(FRAMEWORK_REPO_GPG_KEY_PATH);
		if ($direct['valid']) {
	...
	private function updateRepoKey() {
	...
	public function checkAndUpdateRepoKey($autoUpdate = false, $outputCallback = null) {
		...
		if (is_callable($outputCallback) && !empty($result['message'])) {
			$outputCallback($result['message'], $result['success']);
		}
