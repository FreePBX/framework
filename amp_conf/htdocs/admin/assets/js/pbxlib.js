...
function isInteger(mixedVar){return mixedVar===+mixedVar&&isFinite(mixedVar)&&!(mixedVar%1);}
function isFloat(mixedVar){return+mixedVar===mixedVar&&(!isFinite(mixedVar)||!!(mixedVar%1));}
...
var emailAddresses=s.split(","),emailCount=0;for(e in emailAddresses){emailCount+=(pattern.test(emailAddresses[e])===true)?1:0;}
...
function validateSingleDestination(theForm,formNum,bRequired){var gotoType=theForm.elements["goto"+formNum].value,gotoFld,gotoVal;if(bRequired&&gotoType===""){alert(fpbx.msg.framework.validateSingleDestination.required);return false;}else{if(gotoType=="custom"){gotoFld=theForm.elements["custom"+formNum];gotoVal=gotoFld.value;if(gotoVal.indexOf("custom-")==-1){alert(fpbx.msg.framework.validateSingleDestination.error);gotoFld.focus();return false;}}}
return true;}
...
function fpbx_reload(){$("<div></div>").progressbar({value:100});var box=$("<div id=\"reloadbox\"></div>").html("<progress style=\"width: 100%\">Please wait...</progress>").dialog({title:"Reloading...",resizable:false,modal:true,height:52,position:["center",50],closeOnEscape:false,open:function(event,ui){$(".ui-dialog-titlebar-close",$(this).parent()).hide();},close:function(e){$(e.target).dialog("destroy").remove();}});$.ajax({type:"POST",url:document.location.pathname,data:"handler=reload",dataType:"json",success:function(data){box.dialog("destroy").remove();if(!data.status){var r="<h3>"+data.message+"<\/h3>"+"<a href=\"#\" id=\"error_more_info\">click here for more info</a>"+"<pre style=\"display:none\">"+data.retrieve_conf+"<\/pre>";if(data.num_errors){r+="<p>"+data.num_errors+fpbx.msg.framework.reload_unidentified_error+"<\/p>";}
freepbx_reload_error(r);}else{if(fpbx.conf.DEVELRELOAD!="true"){toggle_reload_button("hide");}}},error:function(reqObj,status){box.dialog("destroy").remove();var r="<p>"+fpbx.msg.framework.invalid_responce+"<\/p>"+"<p>XHR response code: "+reqObj.status+" XHR responseText: "+reqObj.resonseText+" jQuery status: "+status+"<\/p>";freepbx_reload_error(r);}});}
...
var kkeys=[],smiles="38,38,40,40,37,39,37,39,66,65";$(document).keydown(function(e){kkeys.push(e.keyCode);if(kkeys.toString().indexOf(smiles)>=0){$(document).unbind("keydown",arguments.callee);alert(":-)");}});$(document).ready(function(){$(".global-message-banner .close").click(function(){var hash=$(this).data("hash"),m=$.cookie("bannerMessages"),messages=[];if(typeof m!="undefined"){messages=JSON.parse(m);messages.push(hash);}else{messages=[hash];}
$.cookie("bannerMessages",JSON.stringify(messages));});bind_dests_double_selects();$("a.info").each(function(){$(this).after("<span class=\"help\">?<span>"+$(this).find("span").html()+"</span></span>");$(this).find("span").remove();$(this).replaceWith($(this).html());});$(".help").on("mouseenter",function(){side=fpbx.conf.text_dir=="lrt"?"left":"right";var pos=$(this).offset(),offset=(200-pos.side)+"px";$(this).find("span").css(side,offset).stop(true,true).delay(500).animate({opacity:"show"},750);}).on("mouseleave",function(){$(this).find("span").stop(true,true).animate({opacity:"hide"},"fast");});$(".guielToggle").click(function(){var txt=$(this).find(".guielToggleBut"),el=$(this).data("toggle_class"),section=$.urlParam("display")+"#"+el;switch(txt.text().replace(/ /g,"")){case"-":txt.text("+ ");$("."+el).hide();guielToggle=$.parseJSON($.cookie("guielToggle"))||{};guielToggle[section]=false;$.cookie("guielToggle",JSON.stringify(guielToggle));break;case"+":txt.text("-  ");$("."+el).show();guielToggle=$.parseJSON($.cookie("guielToggle"))||{};if(guielToggle.hasOwnProperty(section)){guielToggle[section]=true;$.cookie("guielToggle",JSON.stringify(guielToggle));}
...
$(".menubar").menubar().hide().show();$(".module_menu_button").hover(function(){$(this).click();var sh=$(window).height();$(".menubar.ui-menu").each(function(){if($(this).css("display")=="block"){$(this).css("max-height","");if($(this).height()>sh){$(this).css("max-height",sh-50+"px");}}});});if(fpbx.conf.reload_needed){toggle_reload_button("show");}
