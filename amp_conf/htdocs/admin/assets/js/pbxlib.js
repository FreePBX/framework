...
var popover_box;var popover_box_class;var popover_box_mod;var popover_select_id;var dd_trigger_semaphore=false;function bind_dests_double_selects(){$('.destdropdown').unbind().bind('blur click change keypress',function(e){if(dd_trigger_semaphore){return false;}else{dd_trigger_semaphore=true;}
var id=$(this).data('id');var id=typeof id=='undefined'?'':id;var dest=$(this).val();$('[data-id='+id+'].destdropdown2').hide();dd2=$('#'+dest+id+'.destdropdown2');cur_val=dd2.show().val();if(dd2.children().length>1&&cur_val=='popover'){dd2.val('');cur_val='';}
if(cur_val=='popover'){dd2.trigger('change');}
dd_trigger_semaphore=false;});$('.destdropdown2').unbind().bind('change',function(){var dest=$(this).val();if(dest=="popover"){var urlStr=$(this).data('url')+'&fw_popover=1';var id=$(this).data('id');popover_select_id=this.id;popover_box_class=$(this).data('class');popover_box_mod=$(this).data('mod');popover_box=$('<div id="popover-box-id" data-id="'+id+'"></div>').html('<iframe data-popover-class="'+popover_box_class+'" id="popover-frame" frameBorder="0" src="'+urlStr+'" width="100%" height="95%"></iframe>').dialog({title:'Add',resizable:false,modal:true,position:['center',50],width:window.innerWidth-(window.innerWidth*.10),height:window.innerHeight-(window.innerHeight*.10),create:function(){$("body").scrollTop(0).css({overflow:'hidden'});},close:function(e){var id=$(this).data('id');var par=$('#goto'+id).data('last');$('#goto'+id).val(par).change();if(par!=''){var par_id=par.concat(id);$('#'+par_id).val($('#'+par_id).data('last')).change();}
