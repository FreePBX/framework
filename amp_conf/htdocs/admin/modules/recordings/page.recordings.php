...
if ($action == "recorded") {
	...
	if (recording_add($rname, $filename.".wav")) { 
		$recording_saved_ok = true;
	}
}
...
<div class="rnav">
    <li><a id="<?php echo isset($extdisplay)?'current':'nul'; ?>" href="config.php?display=<?php echo urlencode($display)?>"><?php echo _("Add Recording")?></a></li>
...
if (isset($tresults)){
	foreach ($tresults as $tresult) {
		echo "<li><a id=\"".($recordingdisplay==$tresult ? 'current':'nul')."\" href=\"config.php?display=".$display."&amp;recordingdisplay={$tresult['0']}\">{$tresult['1']}</a></li>\n";
	}
}
...
<?php
if (!empty($usersnum)) {
	echo _("Using your phone,")?> <a href="#" class="info"><?php echo _("dial *77")?><span><?php echo _("Start speaking at the tone. Hangup when finished.")?></span></a> <?php echo _("and speak the message you wish to record.");
	} else { ?>
