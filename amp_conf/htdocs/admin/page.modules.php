...
switch($extdisplay) {
	...
	default:
		echo "<h2>" . _("Module Administration") . "</h2>";
		echo "<a href='config.php?display=modules&amp;type=tool&amp;extdisplay=online'>"._("Check for updates online")."</a>\n";
	...
	switch ($modules[$name]['status']) {
		...
		default:
			...
			if (isset($modules_online[$name]['version'])) {
				...
				if ($vercomp < 0) {
					echo '<input type="radio" id="install_'.prep_id($name).'" name="moduleaction['.prep_id($name).']" value="install" /> '.
			 		     '<label for="install_'.prep_id($name).'">Upgrade to '.$modules_online[$name]['version'].'</label> <br />';
				}
			}
