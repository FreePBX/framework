...
// USE_FREEPBX_MENU_CONF
  ...
  foreach ($module_migrate as $setting => $type) {
	...
	if(in_array($setting,array('USEQUEUESTATE','QUEUES_UPDATECDR'))) {
		$mod_set['value'] = true;
	} else {
		$mod_set['value'] = $val;
	}
  ...
  $set = array('category' => 'Proxy Settings', 'emptyok' => 0, 'readonly' => 0);
  $set['value'] = false;
  ...
  $set['sortorder'] = 2;
  $set['type'] = CONF_TYPE_SELECT;
  $freepbx_confc->define_conf_setting('PROXY_TYPE',$set);
   */
