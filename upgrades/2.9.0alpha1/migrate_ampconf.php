...
if(DB::IsError($result)){
  ...
  // TODO: need to change types on a bunch of these and more likely come up with a different way to do this like use the new
  //       class to insert all this stuff (could get messy but...), look into this!
  $sql[] = "INSERT INTO `freepbx_settings` 
    (keyword, value, level, description, type, options, defaultval, readonly, hidden)
    VALUES 
		...
		('AMPBADNUMBER','1',0,'Generate the bad-number context which traps any bogus number or feature code and plays a message to the effect. If you use the Early Dial feature on some Grandstream phones, you will want to set this to false<br>Default = true','bool',NULL,'1',NULL,NULL),
		('AMPBIN','/var/lib/asterisk/bin',0,'Location of the FreePBX command line scripts<br>Default = /var/lib/asterisk/bin','dir',NULL,'/var/lib/asterisk/bin',NULL,NULL),
		('AMPCGIBIN','/var/www/cgi-bin ',0,'The path to Apache cgi-bin dir (leave off trailing slash)<br>Default = var/www/cgi-bin','dir',NULL,'/var/www/cgi-bin',NULL,NULL),
		...
		('AMPDISABLELOG','1',0,'Whether or not to invoke the FreePBX log facility<br>Default = true','bool',NULL,'1',NULL,NULL),
		('AMPENABLEDEVELDEBUG','0',0,'Whether or not to include log messages marked as <b>devel-debug</b> in the log system<br>Default = false','bool',NULL,'0',NULL,NULL),
		...
		('AMPSBIN','/usr/sbin',0,'Where (root) command line scripts are located<br>Default = /usr/local/sbin','dir',NULL,'/usr/sbin',NULL,NULL),
		('AMPSYSLOGLEVEL','',0,'Where to log if enabled, SQL, LOG_SQL logs to old MySQL table, others are passed to syslog system to determine where to log. Values are: LOG_EMERG, LOG_ALERT, LOG_CRIT, LOG_ERR, LOG_WARNING, LOG_NOTICE, LOG_INFO, LOG_DEBUG, LOG_SQL,SQL<br>Default = LOG_ERR','uint',NULL,NULL,NULL,NULL),
		...
		('AMPWEBROOT','/var/www/html',0,'The path to Apache webroot (leave off trailing slash)<br>Default = /var/www/html','dir',NULL,'/var/www/html',NULL,NULL),
		...
		('ASTAGIDIR','/var/lib/asterisk/agi-bin',0,'This is the default directory for Asterisks agi files<br>Default = /var/lib/asterisk/agi-bin','dir',NULL,'/var/lib/asterisk/agi-bin',NULL,NULL), 
		('ASTETCDIR','/etc/asterisk',0,'This is the default directory for Asterisks configuration files<br>Default = /etc/asterisk','dir',NULL,'/etc/asterisk',NULL,NULL), 
		('ASTLOGDIR','/var/log/asterisk',0,'This is the default directory for Asterisks log files<br>Default = /var/log/asterisk','dir',NULL,'/var/log/asterisk',NULL,NULL), 
		('ASTMODDIR','/usr/lib/asterisk/modules',0,'This is the default directory for Asterisks modules<br>Default = /usr/lib/asterisk/modules','dir',NULL,'/usr/lib/asterisk/modules',NULL,NULL), 
		('ASTSPOOLDIR','/var/spool/asterisk',0,'This is the default directory for Asterisks spool directory<br>Default = /var/spool/asterisk','dir',NULL,'/var/spool/asterisk',NULL,NULL), 
		('ASTRUNDIR','/var/run/asterisk',0,'This is the default directory for Asterisks run files<br>Default = /var/run/asterisk','dir',NULL,'/var/run/asterisk',NULL,NULL), 
		('ASTVARLIBDIR','/var/lib/asterisk',0,'This is the default directory for Asterisks lib files<br>Default = /var/lib/asterisk','dir',NULL,'/var/lib/asterisk',NULL,NULL), 
		('ASTMANAGERPORT','5038',0,'Port for the Asterisk Manager<br>Default = 5038','uint',NULL,'5038',NULL,NULL), 
		...
		('CDRDBPORT','',0,'Only used if you dont use the default values provided by FreePBX.<br>Port number for db host','uint',NULL,NULL,NULL,NULL),
		...
		('CHECKREFERER','1',0,'When set to the default value of true, all requests into FreePBX that might possibly add/edit/delete settings will be validated to assure the request is coming from the server. This will protect the system from CSRF (cross site request forgery) attacks. It will have the effect of preventing legitimately entering URLs that could modify settings which can be allowed by changing this field to false<br>Default = false','bool',NULL,'1',NULL,NULL),
		('CUSTOMASERROR','1',0,'If false, then the Destination Registry will not report unknown destinations as errors. This should be left to the default true and custom destinations should be moved into the new custom apps registry<br>Default = true','bool',NULL,'1',NULL,NULL),
		('CWINUSEBUSY','1',0,'For extensions that have CW enabled, report unanswered CW calls as <b>busy</b> (resulting in busy voicemail greeting). If set to no, unanswered CW calls simply report as <b>no-answer</b><br>Default = true','bool',NULL,'1',NULL,NULL),
		...
		('DEVEL','0',0,'Needs to be documented<br>Default = false','bool',NULL,'0',NULL,NULL),
		('DEVELRELOAD','0',0,'Needs to be documented<br>Default = false','bool',NULL,'0',NULL,NULL),
		('DISABLECUSTOMCONTEXTS','0',0,'Normally FreePBX auto-generates a custom context that may be usable for adding custom dialplan to modify the normal behavior of FreePBX. It takes a good understanding of how Asterisk processes these includes to use this and in many of the cases, there is no useful application. All includes will result in a WARNING in the Asterisk log if there is no context found to include though it results in no errors. If you know that you want the includes, you can set this to true. If you comment it out FreePBX will revert to legacy behavior and include the contexts<br>Default = false','bool',NULL,'0',NULL,NULL),
		('DYNAMICHINTS','0',0,'If true, Core will not statically generate hints, but instead make a call to the AMPBIN php script, and generate_hints.php through an Asterisk #exec call. This requires Asterisk.conf to be configured with <b>execincludes=yes<b> set in the [options] section<br>Default = false','bool',NULL,'0',NULL,NULL),
		('ENABLECW','1',0,'Enable call waiting by default when an extension is created (Default is yes). Set to <b>no</b> to if you dont want phones to be commissioned with call waiting already enabled. The user would then be required to dial the CW feature code (*70 default) to enable their phone. Most installations should leave this alone. It allows multi-line phones to receive multiple calls on their line appearances<br>Default = yes','bool',NULL,'1',NULL,NULL),
		('FCBEEPONLY','0',0,'When set to true, a beep is played instead of confirmation message when activating/de-activating: CallForward, CallWaiting, DayNight, DoNotDisturb and FindMeFollow<br>Default = false','bool',NULL,'0',NULL,NULL),
		('FOPDISABLE','0',0,'Set to true to disable FOP in interface and retrieve_conf.  Useful for sqlite3 or if you dont want FOP<br>Default = not set','bool',NULL,'0',NULL,NULL),
		...
		('FOPRUN','1',0,'Set to true if you want FOP started by freepbx_engine (amportal_start), false otherwise<br>Default = true','bool',NULL,'1',NULL,NULL),
		...
		('FOPWEBROOT','/var/www/html/panel',0,'Path to the Flash Operator Panel webroot (leave off trailing slash)<br>Default = /var/www/html/panel','dir',NULL,'/var/www/html/panel',NULL,NULL),
		...
		('POST_RELOAD_DEBUG','0',0,'Display debug output for script used if POST_RELOAD is used<br>Default = not set','bool',NULL,'0',NULL,NULL),
		('RELOADCONFIRM','1',0,'When set to false, will bypass the confirm on Reload Box<br>Default = true','bool',NULL,'1',NULL,NULL),
		('SERVERINTITLE','0',0,'Precede browser title with the server name<br>Default = false','bool',NULL,'false',NULL,NULL), 
		...
		('USECATEGORIES','1',0,'Controls if the menu items in the admin interface are sorted by category (true) or sorted alphebetically with no categories shown (false). Defaults = true','bool',NULL,'1',NULL,NULL),
		('USEDEVSTATE','1',0,'If this is set, it assumes that you are running Asterisk 1.4 or higher and want to take advantage of the func_devstate.c backport available from Asterisk 1.6. This allows custom hints to be created to support BLF for server side feature codes such as daynight, followme, etc<br>Default = false','bool',NULL,'1',NULL,NULL),
		('USEGOOGLEDNSFORENUM','0',0,'Setting this flag will generate the required global variable so that enumlookup.agi will use Google DNS 8.8.8.8 when performing an ENUM lookup. Not all DNS deals with NAPTR record, but Google does. There is a drawback to this as Google tracks every lookup. If you are not comfortable with this, do not enable this setting. Please read Google FAQ about this: <b>http://code.google.com/speed/public-dns/faq.html#privacy</b><br>Default = false','bool',NULL,'0',NULL,NULL),
